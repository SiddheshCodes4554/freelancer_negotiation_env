...
This copy is placed at the OpenEnv deployment root so validators that check
for inference.py in the deployed repository root can pass reliably.
"""
...
try:
    ...
    from models import FreelancerNegotiationAction as ActionModel
    from models import NegotiationActionType
    from tasks import TaskDefinition, get_tasks, grade_task

    FreelancerNegotiationAction = ActionModel
...
def main() -> None:
    ...
    with client.sync() as env:
        for task in get_tasks():
            run_task(env=env, llm_client=llm_client, model_name=MODEL_NAME, task=task)
